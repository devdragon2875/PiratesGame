...
public class TradeScreen {
	...
	public TradeScreen(PApplet parent, float aPrice, float bPrice, float cPrice, float dPrice) {
		...
		parts[1] = new TradePart(parent, bPrice, parent.width / 5, 0, parent.width / 5, parent.height, spicesImg,
				"Spices", Cargo.SPICES);
		parts[2] = new TradePart(parent, cPrice, parent.width * 2 / 5, 0, parent.width / 5, parent.height, jewelryImg,
				"Jewelry", Cargo.JEWELRY);
		parts[3] = new TradePart(parent, dPrice, parent.width * 3 / 5, 0, parent.width / 5, parent.height, beardsImg,
				"Beards", Cargo.BEARDS);
	...
	public void show(Player player) {
		...
		parent.image(jewelryImg, offset + parent.width / 100, parent.height * 5 / 8, parent.width / 15,
				parent.width / 15);
		parent.image(beardsImg, offset + parent.width / 100, parent.height * 6 / 8, parent.width / 15,
				parent.width / 15);

		parent.fill(0);
		parent.textSize(40);
		parent.textAlign(parent.CENTER);
		parent.text(player.getCargo().getNumCloth(), offset + parent.width / 100 + parent.width / 8,
				parent.height * 3 / 8 + parent.width / 20);
		parent.text(player.getCargo().getNumSpices(), offset + parent.width / 100 + parent.width / 8,
				parent.height / 2 + parent.width / 20);
		parent.text(player.getCargo().getNumJewelry(), offset + parent.width / 100 + parent.width / 8,
				parent.height * 5 / 8 + parent.width / 20);
		parent.text(player.getCargo().getNumBeards(), offset + parent.width / 100 + parent.width / 8,
				parent.height * 6 / 8 + parent.width / 20);
