...
public class DrawingSurface extends PApplet {
	...
	private int screen;
	private static final int MENU = 0, GAME = 1, TRADE = 2, DEAD = 3, SELECT_SERVER = 4;
	// DOCKS AND TRADING
	...
	public void draw() {
		...
		if (screen == GAME) {
			if (client == null) {
				...
				for (int i = 0; i < blocks.length; i++) {
					for (int j = 0; j < blocks[i].length; j++) {
						...
						// DRAWS A SAND(randomly picks between textures)
							...
							noStroke();
							/*
							 * double x = Math.random(); if (x < 0.33) { walls.add(new Block(this, i *
							 * blockSize, j * blockSize, blockSize, blockSize, sand1)); } else if (x < 0.66)
							 * { walls.add(new Block(this, i * blockSize, j * blockSize, blockSize,
							 * blockSize, sand2)); } else { walls.add(new Block(this, i * blockSize, j *
							 * blockSize, blockSize, blockSize, sand3)); }
							 */
					...
					for (int i = -1; i < 2; i++) {
						for (int j = -1; j < 2; j++) {
							if (randomI + i < 0 || randomI + i > blocks.length - 1 || randomJ + j < 0
								...
								playerSpawn = false;
								// System.out.println(randomI + " " + randomJ + " wont work because " +
								// randomI+i + " " + randomJ+j + " is a " + blocks[randomI+i][randomI+j]);
								break;
							}
						}
						if (!playerSpawn)
				...
				// System.out.println(randomI + " " + randomJ + " should work because " +
				// randomI + " " + randomJ + " is a " + blocks[randomI][randomI]);
			...
			for (Block w : walls) {
				if (zoom/*
						 * && w.isTouching(player.getX()-700/scaleFactor, player.getY()-500/scaleFactor,
						 * 1500/scaleFactor, 1050/scaleFactor)
						 */) {
					w.show();
				} else if (!zoom) {
			...
			for (Block w : waterBlocks) {
				if (zoom/*
						 * && w.isTouching(player.getX()-(this.width+200)/(2*scaleFactor),
						 * player.getY()-(this.height+200)/(2*scaleFactor),
						 * (this.width+200)/scaleFactor, (this.height+200)/scaleFactor)
						 */) {
					w.show();
				} else if (!zoom) {
					double distance = Math.sqrt(Math.pow(w.getX() - (player.getX() + player.getWidth() / 2), 2)
		...
		// IF TRADE SCREEN
			...
			if(otherBullets != null)
				System.out.println(this.otherBullets.size());
		...
		} else if (screen == SELECT_SERVER) {
			clear();
			background(135, 206, 250);
			// Box
			fill(255);
			stroke(0);
			rect(0, height / 2 - 60, width, 120, 20);
			// Cursor
		}
