	...
	public void drawNoiseMap(double[][] noiseMap, PApplet drawer) {
		...
		for (int y = 0; y < width; y++) {
			for (int x = 0; x < width; x++) {
				Color color = new Color(
						drawer.lerpColor(Color.BLACK.getRGB(), Color.WHITE.getRGB(), (float) noiseMap[x][y]));
				colorMap[x][y] = color;
			}
		}
		...
		for (int i = 0; i < colorMap.length; i++) {
			for (int j = 0; j < colorMap[i].length; j++) {
				...
				drawer.fill(r, g, b);
				drawer.rect(i * drawer.width / colorMap.length, j * drawer.height / colorMap[i].length,
						drawer.width / colorMap.length, drawer.height / colorMap[i].length);
			}
		}
	}
