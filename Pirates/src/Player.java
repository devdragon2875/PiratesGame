...
public class Player extends Block {
	...
	private float angle;
	private Cargo cargo; // this keeps track of gold and how much of each trade good the player is
							// carrying
	...
	public Player(PApplet parent, float x, float y, float width, float height, int maxHealth) {
		...
		weapons.add(new WeaponSection(x + weaponWidth, y + height / 4 + weaponHeight, weaponWidth, weaponHeight, false,
				parent));
		...
		buttonsW.add(
				new Button(parent, x - 500 / 7 + weaponWidth, y + weaponHeight, weaponWidth, weaponHeight, "Can1"));
	...
	public void show() {
		...
		parent.translate((float) (-this.getX() - this.getWidth() / 2.0),
				(float) (-this.getY() - this.getHeight() / 2.0));
		this.draw();
		for (int i = 0; i < weapons.size(); i++) {
			weapons.get(i).draw();
		}
	...
	public void update(Block[] b) {
		...
		AffineTransform.getRotateInstance(angle + Math.PI / 2, x + width / 2, y + height / 2).transform(originalPoints,
				0, newPoints, 0, 4);
		for (int i = 0; i < 4; i++) {
		...
		float thingVar = 6;
		weapons.get(2).setCenter(x + width / 2 + thingVar * parent.cos(angle - parent.QUARTER_PI),
				y + height / 2 + thingVar * parent.sin(angle - parent.QUARTER_PI));
		weapons.get(3).setCenter(x + width / 2 + thingVar * parent.cos(angle + parent.QUARTER_PI),
				y + height / 2 + thingVar * parent.sin(angle + parent.QUARTER_PI));
		weapons.get(0).setCenter(x + width / 2 + thingVar * parent.cos(angle + 3 * parent.QUARTER_PI),
				y + height / 2 + thingVar * parent.sin(angle + 3 * parent.QUARTER_PI));
		weapons.get(1).setCenter(x + width / 2 + thingVar * parent.cos(angle + 5 * parent.QUARTER_PI),
				y + height / 2 + thingVar * parent.sin(angle + 5 * parent.QUARTER_PI));
	...
	private void draw() {
		...
		parent.triangle(x + width / 40, y + height / 25, x + width / 2, y - height / 3, x + width - width / 40,
				y + height / 25);
